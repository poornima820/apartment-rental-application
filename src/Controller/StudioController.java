...
public class StudioController implements Initializable {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		apartmentDao=new ApartmentDao();
		 rs=apartmentDao.getstudioL();
		 try {
			while(rs.next())
			 {
				 int i=1;
				 studioList.add(rs.getString(i));
				 i++;
			 }
		 ...
		 //addSportsPs(rs);
		 apts2.setItems(FXCollections.observableArrayList(studioList));
...
private void addStudioL(ResultSet rs2) {
	// TODO Auto-generated method stub
	try{
		while(rs2.next())
		{
			int i=1;
			studioList.add(rs2.getString(i));
